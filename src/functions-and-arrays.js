...
function findGreatestDiagonal(arr) {
  let rightDownProduct = 0;
  let leftDownProduct = 0;

  arr.forEach((element, i, arr) => {
    element.forEach((number, index, array) => {
      if (i + 3 < arr.length && arr[i + 3][index + 3] !== undefined) {
        let thisProduct =
          array[index] *
          arr[i + 1][index + 1] *
          arr[i + 2][index + 2] *
          arr[i + 3][index + 3];
        if (thisProduct > rightDownProduct) {
          rightDownProduct = thisProduct;
        }
      }
    });
  });
    ...
    element.forEach((number, index, array) => {
      if (i + 3 < arr.length && arr[i + 3][index - 3] !== undefined) {
        let thisProduct =
          ...
          arr[i + 2][index - 2] *
          arr[i + 3][index - 3];
        if (thisProduct > leftDownProduct) {
          leftDownProduct = thisProduct;
        }
      }
    });
  });

  return leftDownProduct > rightDownProduct
    ? leftDownProduct
    : rightDownProduct;
}
